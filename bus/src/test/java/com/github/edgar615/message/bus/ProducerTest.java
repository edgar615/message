...
public class ProducerTest {
  ...
  public void testScheduler() {
    MockProducerRepository producerDao = new MockProducerRepository();
    RoundRobinWriteStream writeStream = new RoundRobinWriteStream();
    MessageProducerScheduler messageProducerScheduler = MessageProducerScheduler
        .create(producerDao, writeStream, 3000);
    messageProducerScheduler.start();
    MessageProducer producer = MessageProducer
        .create(new ProducerOptions(), writeStream, producerDao);
    producer.start();
    for (int i = 0; i < 10; i++) {
    ...
    producer.close();
    messageProducerScheduler.close();
  }
...
//    MessageProducer producer = new MessageProducerImpl(options, writeStream);
...
//    ProducerOptions options = new ProducerOptions()
//            .setMaxQuota(5);
//    MessageProducer producer = new BlockWriteStream(options, storage, 5);
//    AtomicInteger succeed = new AtomicInteger();
//    AtomicInteger failed = new AtomicInteger();
//    for (int i = 0; i < 11; i++) {
//      try {
